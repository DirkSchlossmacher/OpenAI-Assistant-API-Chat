...
import { addMessage, listMessages, checkRunStatus } from "../services/api";
...
export const fetchAssistantResponse = async (
  ...
  try {
    ...
    do {
      ...
      const progress =
        initialProgress + (fetchCount / maxFetches) * (90 - initialProgress); // Calculate progress as a percentage
      ...
      setStatusMessage(
        `Waiting for assistant response... Current status: ${status}. Time elapsed: ${elapsedTime} seconds.`,
      );
