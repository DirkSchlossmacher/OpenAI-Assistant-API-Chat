...
class ChatManager {
  ...
  public static getInstance(
    ...
    // Add setIsLoadingFirstMessage here
  ...
  async startAssistant(
    ...
    try {
      ...
      const fileIds = files
        ? await Promise.all(
            files.map((file) =>
              prepareUploadFile(file, this.state.setStatusMessage),
            ),
          )
        : [];
      ...
      const runId = await runChatAssistant(
  ...
  async sendMessage(
    ...
    // Add a new parameter for the files
    ...
    try {
      if (this.state.threadId && this.state.assistantId) {
        ...
        await submitUserMessage(
          ...
          this.state.setStatusMessage,
          ChatFileIds,
        ); // Pass the file IDs here
        console.log("User message submitted. Running assistant...");

        // Run the assistant
        this.state.runId = await runChatAssistant(
          this.state.assistantId as string,
          this.state.threadId as string,
        );
        console.log(
          "Assistant run successfully. Fetching assistant response...",
        );
