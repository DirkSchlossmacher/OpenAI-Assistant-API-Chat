...
export const monitoringUpsert = async (
    assistantId: string | undefined,
    param: string | undefined,
    path: string | undefined,
    dateKey: string
    ...
    try {
      await redis.hincrby(`monitoringCount:${assistantId}`, dateKey, 1);
      const timeValue = new Date().toISOString(); // full date
      await redis.hset(`monitoringTimeValue`, { last_sessionInc: timeValue, assitantId: assistantId, path: path, param: param });
    } catch (error) {
      console.error('Failed to increment session refresh count in Redis via Upstash', error);
    }
  };
